...
static NSMutableDictionary * g_mtWatchdog = nil;
static FMDatabase * g_mtMainDB = nil;

// 初始化检查者数据结构
void mtDBCheckerInit()
{
    g_mtWatchdog = [[NSMutableDictionary alloc]init];
}

// 去初始化检查者
void mtDBCheckerFini()
{
    [g_mtWatchdog release];
}

// 设置主线程使用的常开数据库句柄
void mtDBCheckerSetMainHandle(FMDatabase * mainDb)
{
    g_mtMainDB = mainDb;
}

// 检查子线程是否误用主线程句柄
void mtDBMainHandleCheck(FMDatabase * db)
{
    if (NO == [NSThread isMainThread] 
        && db == g_mtMainDB)
    {
        NSException * e = [NSException exceptionWithName:@"mt-error" 
                                                  reason:@"use main db handle on second thread" 
                                                userInfo:nil];
        @throw e; 
    }
}
...
// 设置某个线程的事务使用的数据库句柄
void mtCheckerSet(FMDatabase * db)
{
    if (!g_mtWatchdog)
    {
        return;
    }
    mtDBMainHandleCheck(db);
    NSString * tid = [NSString stringWithFormat:@"%@", [NSThread currentThread]];
    NSString * dbStr = [NSString stringWithFormat:@"%p", db];
    @synchronized(g_mtWatchdog){
        NSString * tdb = [g_mtWatchdog objectForKey:tid];
        if (tdb)
        {
            NSException * e = [NSException exceptionWithName:@"mt-error" 
                                                      reason:@"set db handle on an unfinished transction" 
                                                    userInfo:nil];
            @throw e; 
        }
        [g_mtWatchdog setObject:dbStr forKey:tid];
    }
}

// 清除某个线程事务句柄
void mtCheckerClear(FMDatabase * db)
{
    if (!g_mtWatchdog)
    {
        return;
    ...
    @synchronized(g_mtWatchdog){
        ...
        {
            NSException * e = [NSException exceptionWithName:@"mt-error" 
                                                      reason:@"clear not open db handle of transction" 
                                                    userInfo:nil];
            @throw e;
        }
        if (NO == [tdb isEqualToString:dbStr])
        {
            NSException * e = [NSException exceptionWithName:@"mt-error" 
                                                      reason:@"clear wrong db handle of transction" 
                                                    userInfo:nil];
            @throw e;
        }
        [g_mtWatchdog removeObjectForKey:tid];
    }
}

// 检查某个线程是否在事务中又打开了新的句柄
void mtCheck(FMDatabase * db)
{
    if (!g_mtWatchdog)
    {
        return;
    ...
    @synchronized(g_mtWatchdog){
        NSString * tdb = [g_mtWatchdog objectForKey:tid];
        if (tdb && (NO == [tdb isEqualToString:dbStr]))
        {
            NSException * e = [NSException exceptionWithName:@"mt-error" 
                                    reason:@"open more than one db handle in transction" 
                                  userInfo:nil];
            @throw e;
        }
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    // 增加一个异常打印
    if (rc != SQLITE_OK){
        NSLog(@"Unknown sqlite3 error (%d: %s) eu", rc, sqlite3_errmsg(db));        
    }
